...
const WAREHOUSE_RESET_PURPOSE = "warehouse_reset";
...
function hashOtp(channel, target, otp, purpose = WAREHOUSE_SIGNUP_PURPOSE) {
  return crypto
...
async function incrementWindowCounter(redisKey, { limit, windowSeconds }) {
  ...
  if (!globalThis.__WAREHOUSE_OTP_WINDOW_COUNTER__) {
    globalThis.__WAREHOUSE_OTP_WINDOW_COUNTER__ = new Map();
  }

  const now = Date.now();
  const store = globalThis.__WAREHOUSE_OTP_WINDOW_COUNTER__;
...
function normalizeTarget(channel, rawValue) {
  if (channel === "email") return normalizeEmail(rawValue);
  if (channel === "phone") return normalizePhone(rawValue);
...
async function ensureTargetExists(channel, target) {
  ...
  const existing = await Warehouse.findOne(query).select("_id").lean();
  if (!existing) {
    const error = new Error(
      channel === "email"
        ? "No warehouse found with this email"
        : "No warehouse found with this phone number",
    );
...
async function ensureTargetAvailable(channel, target) {
  ...
  if (existing) {
    const error = new Error(
      channel === "email"
        ? "A warehouse with this email already exists"
        : "A warehouse with this phone number already exists",
    );
    error.statusCode = 400;
    throw error;
  }
...
async function dispatchPhoneOtp({ phone, otp }) {
  ...
  console.log(`[WarehousePhoneOTP][mock] ${phone} -> ${otp}`);
}

function signVerificationToken({ channel, target, purpose = WAREHOUSE_SIGNUP_PURPOSE }) {
  return jwt.sign(
    { purpose, channel, target, verified: true },
    verificationSecret(),
    { expiresIn: process.env.SELLER_VERIFICATION_TOKEN_EXPIRY || "2h" },
  );
}

export function verifyWarehouseVerificationToken({ channel, rawValue, token, purpose = WAREHOUSE_SIGNUP_PURPOSE }) {
...
export async function issueWarehouseVerificationOtp({ channel, rawValue, ipAddress = "unknown" }) {
  const normalizedChannel = String(channel || "").trim().toLowerCase();
  ...
  const sendAllowed = await incrementWindowCounter(
    `warehouse:otp:send:${normalizedChannel}:${target}`,
  ...
  let session = await OtpVerification.findOne({
    purpose: WAREHOUSE_SIGNUP_PURPOSE,
  ...
  const otp = generateWarehouseOtp(normalizedChannel);
  ...
  console.log(JSON.stringify({
    ...
    event: "warehouse_signup_otp_issued",
    channel: normalizedChannel,
  ...
  return {
    sent: true,
    channel: normalizedChannel,
    maskedTarget: normalizedChannel === "email" ? maskEmail(target) : maskPhone(target),
    expiresInSeconds: OTP_EXPIRY_MINUTES() * 60,
  };
}

export async function verifyWarehouseOtpCode({ channel, rawValue, otp, ipAddress = "unknown" }) {
  const normalizedChannel = String(channel || "").trim().toLowerCase();
  const target = normalizeTarget(normalizedChannel, rawValue);
  ...
  const verifyAllowed = await incrementWindowCounter(
    `warehouse:otp:verify:${normalizedChannel}:${target}`,
  ...
  return {
    verified: true,
    channel: normalizedChannel,
    verificationToken: signVerificationToken({ channel: normalizedChannel, target }),
  };
}

export async function issueWarehouseResetOtp({ channel, rawValue, ipAddress = "unknown" }) {
  const normalizedChannel = String(channel || "").trim().toLowerCase();
  const target = normalizeTarget(normalizedChannel, rawValue);
  ...
  const sendAllowed = await incrementWindowCounter(
    `warehouse:reset_otp:send:${normalizedChannel}:${target}`,
    { limit: OTP_SEND_LIMIT_PER_WINDOW(), windowSeconds: OTP_SEND_LIMIT_WINDOW_SECONDS() },
  );
  if (!sendAllowed) {
    const error = new Error("Too many OTP requests. Please try again later.");
  ...
  const now = new Date();
  let session = await OtpVerification.findOne({
    purpose: WAREHOUSE_RESET_PURPOSE,
    channel: normalizedChannel,
    target,
  }).select("+otpHash +expiresAt");
  ...
  if (!session) {
    session = new OtpVerification({
      ...
      channel: normalizedChannel,
      target,
      otpHash: hashOtp(normalizedChannel, target, otp, WAREHOUSE_RESET_PURPOSE),
      expiresAt,
      verifiedAt: null,
      failedAttempts: 0,
      lastSentAt: now,
    ...
    session.otpHash = hashOtp(normalizedChannel, target, otp, WAREHOUSE_RESET_PURPOSE);
    session.expiresAt = expiresAt;
    session.verifiedAt = null;
    session.failedAttempts = 0;
    session.lastSentAt = now;
...
export async function verifyWarehouseResetOtpCode({ channel, rawValue, otp, ipAddress = "unknown" }) {
  const normalizedChannel = String(channel || "").trim().toLowerCase();
  const target = normalizeTarget(normalizedChannel, rawValue);
  const code = String(otp || "").trim();
  ...
  const verifyAllowed = await incrementWindowCounter(
    `warehouse:reset_otp:verify:${normalizedChannel}:${target}`,
    { limit: OTP_VERIFY_LIMIT_PER_WINDOW(), windowSeconds: OTP_VERIFY_LIMIT_WINDOW_SECONDS() },
  );
  if (!verifyAllowed) {
    const error = new Error("Too many verification attempts. Please try again later.");
  ...
  const isValid = hashOtp(normalizedChannel, target, code, WAREHOUSE_RESET_PURPOSE) === session.otpHash;
  if (!isValid) {
    session.failedAttempts = (session.failedAttempts || 0) + 1;
    await session.save();
    if (session.failedAttempts >= OTP_MAX_FAILED_ATTEMPTS()) {
